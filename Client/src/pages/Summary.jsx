...
import { dummySummaryData } from "../data/dummyData";

function Summary() {
  const [search, setSearch] = useState("");

  const filteredData = useMemo(() => {
    if (!search.trim()) return dummySummaryData;

    const query = search.toLowerCase();
    return dummySummaryData.filter(
      (entry) =>
        entry.examinerName.toLowerCase().includes(query) ||
            ...
            <tbody>
              {filteredData.map((entry) => (
                <tr key={entry._id} className="border-t border-slate-200">
                  <td className="px-4 py-3">{entry.examinerName}</td>
